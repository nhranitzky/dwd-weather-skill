...
```bash
make test      # runs the test suite (44 tests)
...
```
dwd-weather-skill/
├── Makefile                        ← dev tasks: install, lint, test, package, deploy
├── README.md
├── images/
...
└── dwd-weather/
    ...
    │   ├── utils.py                ← geocoding, API calls, formatting, cache
    ...
    │   ├── test_utils.py           ← unit tests (aggregate, formatters, geocode, API)
...
| `No data for this location` | No DWD station near the coordinates; try a German city |
| `Rate limit exceeded` | Too many requests to BrightSky; wait a moment and retry |
| `Service unavailable` | BrightSky is temporarily down; retry after a few minutes |
...
- Geocoding results are cached locally at `~/.cache/skills/dwd-weather/` (TTL 7 days).
